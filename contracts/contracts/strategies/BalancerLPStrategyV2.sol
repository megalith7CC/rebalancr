...
contract BalancerLPStrategyV2 is BaseStrategyAgent {
	using SafeERC20 for IERC20;

	IProtocolAdapter public immutable balancerAdapter;
	...
	constructor(address _balancerAdapter, address _priceOracle) Ownable(msg.sender) {
		InputValidation.validateNotZeroAddress(_balancerAdapter, 'balancerAdapter');
		InputValidation.validateNotZeroAddress(_priceOracle, 'priceOracle');

		balancerAdapter = IProtocolAdapter(_balancerAdapter);
	...
	function getTokenAPY(address token) public view returns (uint256 apy) {
		if (!balancerAdapter.isTokenSupported(token)) {
			return 0;
		}
		return balancerAdapter.getCurrentAPY(token);
	}
	...
	function getAverageAPY() public view returns (uint256 averageAPY) {
		...
		for (uint256 i = 0; i < supportedTokens.length; i++) {
			if (balancerAdapter.isTokenSupported(supportedTokens[i])) {
	...
	function _validateParams(bytes calldata data) internal view override returns (bool) {
		...
		if (!isTokenSupported(token) || !balancerAdapter.isTokenSupported(token)) {
			return false;
		}
		...
		if (!isDeposit) {
			address receiptToken = balancerAdapter.getReceiptToken(token);
	...
	function _onPositionRebalanced(
		...
		address oldReceiptToken = balancerAdapter.getReceiptToken(oldToken);
	...
	function _updateTVL() internal override {
		...
		for (uint256 i = 0; i < supportedTokens.length; i++) {
			...
			if (!balancerAdapter.isTokenSupported(token)) continue;
	...
	function _depositToAdapter(address token, uint256 amount) internal returns (bool success) {
		...
		if (!balancerAdapter.isTokenSupported(token)) revert TokenNotSupported(token);

		IERC20(token).approve(address(balancerAdapter), amount);

		try balancerAdapter.deposit(token, amount, '') returns (bool depositSuccess, address receiptToken, uint256 receiptAmount) {
			if (!depositSuccess) {
	...
	function _withdrawFromAdapter(address token, uint256 amount) internal returns (bool success) {
		...
		try balancerAdapter.withdraw(receiptToken, amount, '') returns (
