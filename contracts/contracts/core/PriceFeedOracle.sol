...
contract PriceFeedOracle is IPriceOracle, Ownable {
	...
	function getLatestPrice(address token) external view override returns (uint256 price) {
		...
		return ChainlinkConsumer.getLatestPrice(AggregatorV3Interface(priceFeed));
	}

	function getTokenPrice(address token, address denomination) external view override returns (uint256 price) {
		...
		if (denominationFeed == address(0)) revert PriceFeedNotFound(denomination);

		uint256 tokenPriceInUsd = ChainlinkConsumer.getLatestPrice(AggregatorV3Interface(tokenFeed));
		uint256 denominationPriceInUsd = ChainlinkConsumer.getLatestPrice(AggregatorV3Interface(denominationFeed));
