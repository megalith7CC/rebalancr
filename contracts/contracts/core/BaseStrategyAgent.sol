...
abstract contract BaseStrategyAgent is IStrategyAgent, Ownable, ReentrancyGuard {
	...
	function entryPosition(
		address token,
		uint256 amount
	) external override onlyInitialized onlyActive supportedToken(token) nonReentrant returns (uint256) {
		...
		uint256 positionId = positionManager.openPosition(strategyInfo.id, token, amount, positionData);
	...
	function exitPosition(uint256 positionId) external override onlyInitialized nonReentrant returns (bool) {
	...
	function rebalancePosition(
		uint256 positionId,
		bytes calldata data
	) external override onlyInitialized onlyActive nonReentrant returns (bool) {
		IPositionManager.Position memory position = positionManager.getPosition(positionId);
		if (position.strategyId != strategyInfo.id) {
			revert Unauthorized(msg.sender);
		}
	...
	function updatePerformanceMetrics(uint256 newTVL, uint256 newAPY, uint256 newRiskScore) external onlyOwner onlyInitialized {
	...
	function _updateMetrics(uint256 newTVL, uint256 newAPY, uint256 newRiskScore) internal {
		tvl = newTVL;
		currentApy = newAPY;
	...
	) internal virtual returns (bytes memory exitData);
